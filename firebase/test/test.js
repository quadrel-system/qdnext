...
beforeEach(async () => {
  ...
  await adminDb.collection(ADMINS).doc(ADMIN_USER_ID).set({ email: ADMIN_USER_EMAIL });
});
...
describe('Tests', () => {
  it('Only users can read & edit their user_settings', async () => {
    ...
    await firebase.assertSucceeds(
      ownerDb.collection(USER_SETTINGS).doc(TEST_USER_ID).set({ darkTheme: true })
    );

    await firebase.assertSucceeds(ownerDb.collection(USER_SETTINGS).doc(TEST_USER_ID).get());

    await firebase.assertSucceeds(
      ownerDb.collection(USER_SETTINGS).doc(TEST_USER_ID).update({ darkTheme: false })
    );

    await firebase.assertFails(usurperDb.collection(USER_SETTINGS).doc(TEST_USER_ID).get());

    await firebase.assertFails(
      usurperDb.collection(USER_SETTINGS).doc(TEST_USER_ID).update({ darkTheme: false })
    );
  });
  ...
  it('Owner can update a project', async () => {
    ...
    await firebase.assertSucceeds(
      ownerDb.collection(PROJECTS).doc(TEST_PROJECT_ID).update({
        name: editName,
      })
    );
  });
  ...
  it('Non owner cannot update a project', async () => {
    ...
    await firebase.assertFails(
      usurperDb.collection(PROJECTS).doc(TEST_PROJECT_ID).update({ name: editName })
    );
  });
  ...
  it('Owner can delete a project', async () => {
    ...
    await firebase.assertSucceeds(ownerDb.collection(PROJECTS).doc(TEST_PROJECT_ID).delete());
  });
  ...
  it('Non owner cannot delete a project', async () => {
    ...
    await firebase.assertFails(usurperDb.collection(PROJECTS).doc(TEST_PROJECT_ID).delete());
  });
  ...
  it('Owner can create/update/delete project contents', async () => {
    ...
    await firebase.assertSucceeds(
      ownerDb.collection(PROJECT_CONTENTS).doc(TEST_PROJECT_CONTENTS_ID).set({
        state: 'test',
        type: 'Element',
    ...
    await firebase.assertSucceeds(
      ownerDb.collection(PROJECT_CONTENTS).doc(TEST_PROJECT_CONTENTS_ID).update({ state: 'test2' })
    );
    // Owner deletes project_contents
    await firebase.assertSucceeds(
      ownerDb.collection(PROJECT_CONTENTS).doc(TEST_PROJECT_CONTENTS_ID).delete()
    );
  });
  ...
  it('Non-owner cannot create project contents', async () => {
    ...
    await firebase.assertFails(
      usurperDb.collection(PROJECT_CONTENTS).doc(TEST_PROJECT_CONTENTS_ID).set({
        state: 'test',
        type: 'Element',
        projectId: TEST_PROJECT_ID,
      })
  ...
  it('Non-owner cannot delete project contents', async () => {
    ...
    await firebase.assertFails(
      usurperDb.collection(PROJECT_CONTENTS).doc(TEST_PROJECT_CONTENTS_ID).delete()
    );
  });
  ...
  it('Add, Remove, and Resolve comments', async () => {
    ...
    await firebase.assertSucceeds(ownerDb.collection(COMMENTS).doc(COMMENT_ID).delete());
    ...
    await firebase.assertSucceeds(
      userTwoDb.collection(COMMENTS).doc(COMMENT_ID).update({ text: 'Edited comment' })
    );

    // Another user DELETES a COMMENT
    await firebase.assertSucceeds(userTwoDb.collection(COMMENTS).doc(COMMENT_ID).delete());

    // Owner DELETES a COMMENT THREAD
    await firebase.assertSucceeds(ownerDb.collection(COMMENTS).doc(COMMENT_THREAD_ID).delete());
    ...
    await firebase.assertSucceeds(
      ownerDb.collection(COMMENTS).doc(COMMENT_THREAD_ID).update({ resolved: true })
    );
    ...
    await firebase.assertSucceeds(
      userTwoDb.collection(COMMENTS).doc(COMMENT_THREAD_ID).update({ resolved: true })
    );
  });
  ...
  it('Admin edits existing changelog entry', async () => {
    ...
    // Admin creates a changelog entry
    await firebase.assertSucceeds(
      adminDb.collection(NEWS_UPDATES).doc(TEST_CHANGELOG_ENTRY_ID).update({
  ...
  it('Admin deletes a changelog entry', async () => {
    ...
    // Admin creates a changelog entry
    await firebase.assertSucceeds(
      adminDb.collection(NEWS_UPDATES).doc(TEST_CHANGELOG_ENTRY_ID).delete()
    );
  });
  ...
  it('Non-admin cannot edit an existing changelog entry', async () => {
    ...
    // Admin creates a changelog entry
    await firebase.assertFails(
      usurperDb.collection(NEWS_UPDATES).doc(TEST_CHANGELOG_ENTRY_ID).update({
        title: 'New Test Entry',
      })
    );
  });
  ...
  it('Non-admin cannot delete a changelog entry', async () => {
    ...
    // Admin creates a changelog entry
    await firebase.assertFails(
      usurperDb.collection(NEWS_UPDATES).doc(TEST_CHANGELOG_ENTRY_ID).delete()
    );
  });
});
